...
diam_avg = nanmean(diam_all);
diam_std = nanstd(diam_all);
N = length(diam_all);
...
% figure('position', [100, 100, 1280, 800]); %Position of figure
...
xlim([0 10]); % X limits
% set(gcf,'FontSize',16)
set(gca,'FontSize',16) %here again? i am stupid
dim   = [.275 .3  .8 .6];%Defines position and height of the text box
...
%Define bin centres and save these and concentrations to csv file
 csvwrite(['Concentration.csv'],[BinCentre.',BinConcentration]);
 % Save histogram as .tif and as matlab figure
TifFile = fullfile(myFolder, 'Histogram.tif');
...
function bw_thresh = binarythresh(gray)
    ...
    global binarysens
    binarysens = 0.5;
   figure(); set(gcf, 'Position', [100, 100, 1280, 800])
     %Create slider
    sld = uicontrol('Style', 'slider','Min',0,'Max',1,'Value',0.5,...
        'Position', [100 20 500 20],'Callback', @thresh1); 
					
    % Add a text uicontrol to label the slider.
    txt = uicontrol('Style','text',...
        'Position',[400 45 120 20],...
        'String','Binarisation Threshold');
...
function circle_thresh(bw,scaling)
   ...
   % Create slider
    sld = uicontrol('Style', 'slider',...
        ...
        'Position', [100 20 500 20],...
        'Callback', @thresh1); 
					
    % Add a text uicontrol to label the slider.
    txt = uicontrol('Style','text',...
        'Position',[400 45 120 20],...
        'String','Circle Sensitivity');
...
fprintf(fileID,'%s %f' , ' Average Diameter = ', diam_avg);
